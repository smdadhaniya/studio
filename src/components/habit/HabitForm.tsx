...
import { HABIT_COLORS, PREDEFINED_MEASUREMENT_UNITS, HABIT_LUCIDE_ICONS_LIST } from '@/lib/constants';
...
export function HabitForm({ onSubmit, initialData, onCancel }: HabitFormProps) {
  ...
  const SelectedIconComponent = HABIT_LUCIDE_ICONS_LIST.find(i => i.name === selectedIconName)?.icon;
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex flex-col sm:flex-row gap-4 items-start">
          ...
          <FormField
            ...
            name="icon"
            render={({ field }) => (
              <FormItem className="w-full sm:w-auto sm:min-w-[180px] pt-0 sm:pt-[28px]"> {/* Adjusted padding for alignment */}
                {/* <FormLabel>Icon</FormLabel> */}
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger className="text-sm">
                      {SelectedIconComponent && <SelectedIconComponent className="w-4 h-4 mr-2 shrink-0" />}
                  ...
                  <SelectContent>
                    <ScrollArea className="h-[200px]">
                      {HABIT_LUCIDE_ICONS_LIST.map((iconItem) => {
                        ...
                        return (
                          <SelectItem key={iconItem.name} value={iconItem.name} className="text-sm">
