...
export function HabitReportModal({
  ...
  const IconComponent = useMemo(() => {
    if (habit?.icon && typeof habit.icon === "string") {
      return HABIT_LUCIDE_ICONS_LIST.find((item) => item.name === habit.icon)
        ?.icon;
    }
  ...
  const averageValuePerCompletion = useMemo(() => {
    if (habit?.trackingFormat === "measurable") {
      const completedWithValue = habitDailyProgress.filter(
        (p) => p.completed && typeof p.value === "number"
      );
  ...
  const successPieData = useMemo(() => {
    ...
    return [
      {
        name: "Completed",
        value: overallSuccessData.completedDays,
        fill: PIE_CHART_COLORS[0],
      },
      {
        name: "Missed",
        value: Math.max(0, overallSuccessData.missedDays),
        fill: PIE_CHART_COLORS[1],
      }, // Ensure missed is not negative
    ];
  }, [overallSuccessData]);
  ...
  const dayOfWeekChartConfig = {
    completions: { label: "Completions", color: CHART_COLORS[0] },
  };
  ...
  const monthlyTrendChartConfig = {
    completions: { label: "Completions", color: CHART_COLORS[1] },
  };
  ...
  const recentValuesChartConfig = {
    ...
    ...(habit?.targetCount !== undefined
      ? { target: { label: "Target", color: CHART_COLORS[4] } }
      : {}),
  };
  ...
  const currentMonthDays = useMemo(() => {
    ...
    return eachDayOfInterval({ start, end }).map((day) => {
      const progressOnDay = habitDailyProgress.find((p) =>
  ...
  if (!habit || !clientRendered) {
    // Ensure client has rendered before trying to display charts
    return null;
  }
          ...
          <div className="py-4 grid gap-6">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 text-sm">
                ...
                <div>
                  <strong className="text-muted-foreground">Tracking:</strong>{" "}
                  {habit.trackingFormat === "yes/no" ? "Yes/No" : `Measurable`}
                </div>
                {habit.trackingFormat === "measurable" && (
                  <div>
                    <strong className="text-muted-foreground">Target:</strong>{" "}
                    {habit.targetCount || "N/A"} {habit.measurableUnit || ""}
                  </div>
                )}
                <div>
                  <strong className="text-muted-foreground">Created:</strong>{" "}
                  {format(parseISO(habit.createdAt), "PPP")}
                </div>
                ...
                <div>
                  ...
                  {overallSuccessData.successRate}%
                </div>
                ...
                {averageValuePerCompletion && (
                  <div>
                    <strong className="text-muted-foreground">
                      Avg. Value:
                    </strong>{" "}
                    {averageValuePerCompletion} {habit.measurableUnit}
                  </div>
                )}
                <div>
                  <strong className="text-muted-foreground">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {overallSuccessData.completedDays} completed days out of{" "}
                    {overallSuccessData.totalTrackedDays} tracked.
                  </CardDescription>
                </CardHeader>
                <CardContent className="h-[200px] md:h-[250px]">
                  {successPieData.length > 0 &&
                  overallSuccessData.totalTrackedDays > 0 ? (
                    <ChartContainer
                      config={successPieChartConfig}
                    ...
                    <p className="text-sm text-muted-foreground text-center pt-10">
                      Not enough data for success rate.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Completions over the last 6 months.
                </CardDescription>
              </CardHeader>
            ...
            {habit.trackingFormat === "measurable" && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Logged values for the last 30 completed entries vs target
                    (if set).
                  </CardDescription>
                </CardHeader>
                <CardContent className="h-[250px]">
                  {recentValuesData.length > 1 ? (
                    <ChartContainer
                      config={recentValuesChartConfig}
                      className="w-full h-full"
                    >
                      <LineChart
                        data={recentValuesData}
                        margin={{ left: 0, right: 10, top: 5, bottom: 5 }}
                      >
                        <CartesianGrid vertical={false} strokeDasharray="3 3" />
                        <XAxis
                        ...
                        <Line
                          type="monotone"
                          dataKey="value"
                          name={habit.measurableUnit || "Value"}
                    ...
                    <p className="text-sm text-muted-foreground text-center pt-10">
                      Not enough measurable data for a trend.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {format(displayedCalendarMonth, "MMMM yyyy")}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-7 gap-1 text-xs text-center">
                    ...
                    {Array.from({
                      length: getDay(dfnsStartOfMonth(displayedCalendarMonth)),
                    }).map((_, i) => (
                      <div
                        key={`empty-${i}`}
                    ...
                    {currentMonthDays.map((dayEntry, index) => (
                      <div
                        key={index}
                        title={
                          format(dayEntry.date, "PPP") +
                          (dayEntry.isCompleted &&
                          habit.trackingFormat === "measurable" &&
                          dayEntry.value
                            ? ` - ${dayEntry.value} ${habit.measurableUnit}`
                            : "")
                        }
                        className={cn(
                      ...
                      <ul className="list-none space-y-0.5">
                        {streakHistory.map((s, i) => (
                          <li key={i} className="flex items-center">
                            ...
                            {s.length} day{s.length === 1 ? "" : "s"} (ended{" "}
                            {format(parseISO(s.endDate), "MMM d, yy")})
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-xs text-muted-foreground">
                        No significant streak history yet.
                      </p>
                    )}
                  </div>
                </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-xs">
                  Showing up to 100 most recent completed entries.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <ScrollArea className="h-[200px] pr-2">
                    <ul className="space-y-1.5">
                      {completedEntriesLog.map((entry) => (
                        <li
                          key={entry.date}
                          className="flex items-center justify-between text-xs p-1.5 rounded-md bg-muted/50 hover:bg-muted/70"
                        >
                          ...
                          {habit.trackingFormat === "measurable" &&
                            entry.value !== undefined && (
                              <span className="font-medium text-foreground">
                                {entry.value} {habit.measurableUnit}
                              </span>
                            )}
                        </li>
                      ))}
