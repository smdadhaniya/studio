      ...
      <DialogContent className="sm:max-w-[600px] bg-card text-card-foreground">
        ...
        >
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger
              value="custom"
              disabled={!!editingHabit && activeTab !== "custom"}
            >
              Create Custom
            </TabsTrigger>
            <TabsTrigger value="presets" disabled={!!editingHabit}>
              Add from Presets
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="presets" className="mt-4">
            {editingHabit ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                Preset selection is disabled when editing an existing habit.
              </p>
            ) : (
              <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {PRESET_HABITS.map((habit) => {
                      const isSelected = selectedPresets.some(
                        (sh) => sh.title === habit.title
                      );
                      const IconComponent = HABIT_LUCIDE_ICONS_LIST.find(
                        (i) => i.name === habit.icon
                      )?.icon;
                      const checkboxId = `preset-create-${habit.title.replace(
                        /\s+/g,
                        "-"
                      )}`;
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleTogglePreset(habit, !!checked)
                            }
                ...
                <div className="flex justify-end gap-2 pt-4">
                  ...
                  <Button
                    type="button"
                    onClick={handlePresetFormSubmit}
