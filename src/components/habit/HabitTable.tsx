...
import { parseDate } from '@/lib/dateUtils'; // getTodayDateString might not be needed here
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      <table className="min-w-full w-max border-collapse border border-border">
        ...
        <tbody>
          {habits.map(habit => {
            const habitProgressForMonth = useMemo(() => {
              const progress = allProgress[habit.id] || [];
              const map = new Map<string, DailyProgress>();
              progress.forEach(p => map.set(p.date, p));
              return map;
            }, [allProgress, habit.id]);

            return (
              <tr key={habit.id} className="group hover:bg-muted/10">
                ...
                {daysInMonth.map(day => {
                  ...
                  const dayProgress = habitProgressForMonth.get(dateStr);
